...
`scopes.json`에는 키별로 마지막 관련 배치의 커밋 근거를 유지합니다. 무관한 다음 배치 때문에 기존 근거가 사라져 검토 상태가 바뀌는 일을 막기 위한 상태입니다. `external.json`에는 현재 배치, 유지한 키별 근거, 원고의 명시적 인용이 참조하는 항목만 남깁니다. 다만 유지 대상 이슈에 `confluenceUrls`가 없으면 링크 관계를 판별할 수 없으므로 기존 Confluence 페이지를 보수적으로 보존합니다. 이 경우 추가 페이지도 프롬프트에 남을 수 있으며, 링크 정보가 확보된 이후 배치에서 다시 정리합니다. `record-missing`으로 Jira 조회 실패를 기록할 때도 이전 링크 목록이 있으면 유지합니다. 이 정리는 커밋이 있는 배치를 수집할 때 실행됩니다. 기존 전체 배치 해시에서 전환할 때는 한 번 재검토가 필요할 수 있으며, 실행기가 검토 승인이나 checkpoint를 자동 변경하지 않습니다.
...
| `DOCGEN_QWEN_NUM_PREDICT` | Ollama 직접 호출의 출력 토큰 한도이며 기본 8,192입니다. 근거 요약 호출은 요약 상한 근처로 더 낮게 묶습니다. 한도에 닿은(`length`) 응답은 요약 재시도나 반려 재요청으로 돌아가고, 다른 미완성 응답은 실패합니다. |
