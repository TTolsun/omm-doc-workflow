...
if ((CONFIG.agent?.kind ?? 'hermes') === 'hermes') {
  const result = spawnSync(process.env.DOCFLOW_HERMES_CLI ?? 'hermes', ['chat', '--help'], { encoding: 'utf8', timeout: 10000, windowsHide: true });
  if (result.status !== 0 || !result.stdout.includes('--query-file')) throw new Error('Hermes CLI의 --query-file 지원 여부를 확인하세요.');
